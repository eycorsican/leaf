...
async fn health_check_task(
    ...
    let measure = async move {
        ...
        let transport = match crate::proxy::connect_datagram_outbound(&sess, dns_client, &h).await {
            Ok(t) => t,
            Err(_) => return Measure(i, u128::MAX),
        };
        let uh = if let Ok(uh) = h.datagram() {
            uh
        } else {
...
impl OutboundDatagramHandler for Handler {
    ...
    async fn handle<'a>(
        ...
        if schedule.is_empty() && self.last_resort.is_some() {
            ...
            let handle = async {
                let transport =
                    crate::proxy::connect_datagram_outbound(sess, self.dns_client.clone(), lr).await?;
                lr.datagram()?.handle(sess, transport).await
            };
            return handle.await;
        }
        ...
        for i in schedule {
            ...
            let handle = async {
                let transport = crate::proxy::connect_datagram_outbound(
                    sess,
                    self.dns_client.clone(),
                    &self.actors[i],
                )
                .await?;
                self.actors[i].datagram()?.handle(sess, transport).await
            };
